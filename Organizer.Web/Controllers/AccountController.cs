...
using Organizer.Web.Controllers.Results;
using Organizer.Web.Models.Account;
...
{
    public class AccountController : OrganizerControllerBase
    {
        ...
        {
            return ForgotPasswordViewModel(new ForgotPasswordViewModel());
        }

        private ActionResult ForgotPasswordViewModel(ForgotPasswordViewModel model)
        {
        ...
        public async Task<JsonResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            ...
            {
                return Json(new AjaxResponse { Result = "Wrong Email", Success = false });
            }
            ...
            var returnUrl = Request.ApplicationPath;
            return Json(new AjaxResponse {Result="Succcess", TargetUrl = returnUrl, Success=true });
            ...
            if (tenancyName.IsNullOrEmpty())
            {
                ...
                switch (tenants.Count)
                {
                    case 0:
                        ...
                        { 
                        var localUser = await _userManager.FindByEmailAsync(loginInfo.Email);
                            if (localUser != null)
                            {
                                ...
                                await _userManager.AddLoginAsync(localUser.Id, info.Login);

                                await _logInManager.LoginAsync(loginInfo.Login, tenancyName);
                                if (string.IsNullOrWhiteSpace(returnUrl))
                                {
