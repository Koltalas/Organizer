...
using Organizer.Migrations.SeedData;
...
namespace Organizer.Tests
{
    public abstract class OrganizerTestBase : AbpIntegratedTestBase<OrganizerTestModule>
    {
        ...
        protected void UsingDbContext(Action<OrganizerDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<OrganizerDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<OrganizerDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<OrganizerDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<OrganizerDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<OrganizerDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<OrganizerDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<OrganizerDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<OrganizerDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<OrganizerDbContext, Task<T>> func)
        {
            T result;
